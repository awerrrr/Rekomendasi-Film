...
# ## Solution Approach
# 
# 1. Menerapkan Content-Based Filtering.
# 2. Menerapkan Collaborative-Based Filtering.
# 3. Menggunakan teknik TF-IDF Vectorization untuk mengekstraksi representasi fitur dari genre film.
# 4. Menghitung kemiripan antar film dengan Cosine Similarity untuk menghasilkan daftar rekomendasi.
# 5. Mengevaluasi hasil rekomendasi dengan metode manual dan contoh nyata (studi kasus pengguna yang menyukai satu film, sistem menyarankan film-film serupa).
...
user_liked_dict = (
    df_merged[df_merged["rating"] >= liked_threshold]
...
def precision_at_k(user_id, k=10):
    liked_movies = user_liked_dict.get(user_id, set())
    
    valid_movies = [
        m for m in liked_movies
        if m in dataset_content["movieId"].values
    ]
    
    if len(valid_movies) < 2:
        return None

    query_movie_id = list(valid_movies)[0]
    relevant_movies = set(valid_movies[1:])
    
    query_label = dataset_content.loc[dataset_content["movieId"] == query_movie_id, "movie_label"].values[0]
    
    recommended = (
        cosine_sim_df[query_label]
    ...
    recommended_ids = set(
        dataset_content[dataset_content["movie_label"].isin(recommended)]["movieId"].values
    )
    
    # Hitung precision@K
    true_positives = relevant_movies & recommended_ids
    return len(true_positives) / k


user_ids = list(user_liked_dict.keys())
scores = [precision_at_k(uid, k=10) for uid in user_ids]
...
if scores:
    ...
    print("Tidak ada user yang memenuhi syarat evaluasi (minimal 2 liked films).")
...
plt.title("Metrik Evaluasi pada Model Collaborative Content Based")
...
get_ipython().system('jupyter nbconvert --to python --no-prompt terapanfix.ipynb')
